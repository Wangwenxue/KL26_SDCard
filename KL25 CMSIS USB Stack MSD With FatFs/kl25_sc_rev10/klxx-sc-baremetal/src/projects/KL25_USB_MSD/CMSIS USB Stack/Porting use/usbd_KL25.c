...
//inline static void protected_and (uint32_t *addr, uint32_t val) { while(__STREXH((__LDREXH(addr) & val),addr)); }
//inline static void protected_or  (uint32_t *addr, uint32_t val) { while(__STREXH((__LDREXH(addr) | val),addr)); }
//inline static void protected_xor (uint32_t *addr, uint32_t val) { while(__STREXH((__LDREXH(addr) ^ val),addr)); }
inline static void protected_and (uint32_t *addr, uint32_t val) { *addr = ((uint32_t)(*addr)) & val; }
inline static void protected_or  (uint32_t *addr, uint32_t val) { *addr = ((uint32_t)(*addr)) | val; }
inline static void protected_xor (uint32_t *addr, uint32_t val) { *addr = ((uint32_t)(*addr)) ^ val; }
...
//        printf("USB  Init failed, clock must be 96M or 48M\r\n");
    ...
    USB0_BDTPAGE1 = (uint8_t) ((uint32_t) BD >> 8 );
    USB0_BDTPAGE2 = (uint8_t) ((uint32_t) BD >> 16);
    USB0_BDTPAGE3 = (uint8_t) ((uint32_t) BD >> 24);

    USB0_ISTAT   = 0xFF;                 /* clear interrupt flags              */

    /* enable interrupts                                                        */
    USB0_INTEN = USB_INTEN_USBRSTEN_MASK | USB_INTEN_TOKDNEEN_MASK | USB_INTEN_SLEEPEN_MASK  |
#ifdef __RTX
...
void USBD_Connect (uint32_t con) {
  ...
  if ( con) {
    USB0_CTL  |= USB_CTL_USBENSOFEN_MASK;            /* enable USB           */
    USB0_CONTROL = USB_CONTROL_DPPULLUPNONOTG_MASK;  /* pull up on D+        */
  }
  else {
    USB0_CTL  &= ~USB_CTL_USBENSOFEN_MASK;           /* disable USB          */
    USB0_CONTROL &= ~USB_CONTROL_DPPULLUPNONOTG_MASK;/* pull down on D+      */
  }
}
...
void USBD_Reset (void) {
  ...
  for (i = 1; i < 16; i++) {
    USB_ENDPT_REG(USB0_BASE_PTR,i) = 0x00;
  }
  ...
  USB_ENDPT_REG(USB0_BASE_PTR,0) = USB_ENDPT_EPHSHK_MASK | /* enable ep handshaking  */
  ...
  USB0_ISTAT   =  0xFF;                /* clear all interrupt status flags   */
  USB0_ERRSTAT =  0xFF;                /* clear all error flags              */
  USB0_ERREN   =  0xFF;                /* enable error interrupt sources     */
  USB0_ADDR    =  0x00;                /* set default address                */
...
void USBD_EnableEP (uint32_t EPNum) {
  ...
  if (EPNum & 0x80) {
    EPNum &= 0x0F;
    USB_ENDPT_REG(USB0_BASE_PTR,EPNum) |= USB_ENDPT_EPHSHK_MASK | /*en ep handshaking*/
...
void USBD_DisableEP (uint32_t EPNum) {
  if (EPNum & 0x80) {
    EPNum &= 0x0F;
    USB_ENDPT_REG(USB0_BASE_PTR,EPNum) &= ~(USB_ENDPT_EPHSHK_MASK |/*dis handshaking */
...
void USBD_SetStallEP (uint32_t EPNum) {
  EPNum &= 0x0F;
  USB_ENDPT_REG(USB0_BASE_PTR,EPNum) |= USB_ENDPT_EPSTALL_MASK;
}
...
void USBD_ClrStallEP (uint32_t EPNum) {
  USB_ENDPT_REG(USB0_BASE_PTR,EPNum&0x0F) &= ~USB_ENDPT_EPSTALL_MASK;
  USBD_ResetEP (EPNum);
}
...
uint32_t USBD_GetFrame (void) {
  return((USB0_FRMNUML | (USB0_FRMNUMH << 8) & 0x07FF));
}
