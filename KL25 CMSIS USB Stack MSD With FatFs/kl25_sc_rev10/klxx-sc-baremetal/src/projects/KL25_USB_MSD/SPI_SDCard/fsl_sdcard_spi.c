                                            ...
                                            uint32_t timeout)
{
    ...
    if (req->cmdIndex == kStopTransmission) //  豕?1?那?CMD12??芍?㏒??迄﹞⊿?赤0xFF ???
    {
        SpiSendByte(0xFF);
    }
    /* Wait for the response coming, the left most bit which is transfered first in response is 0 */
    ...
    {
        response = SpiSendByte(0xFF);
        if (!(response & 0x80)) // 足?3?米?足??t那?那?℅????a0
        {
            break;
        }
    }
    ...
    switch(req->respType)
    {
        ...
        case kSdSpiRespTypeR1b:
        {
            uint8_t busy = 0;
            uint32_t time=0;
            while (busy != 0xFF) //?芍那?米?足??t那?㏒o1)3?那㊣ 2㏒?busy=0xFF
            {
                busy = SpiSendByte(0xFF);
                if (time++ > timeout)
                {
...
static uint32_t SDSPI_DRV_Write(uint8_t *buffer, uint32_t size, uint8_t token)
{
    ...
    if (SpiSendFrame(buffer, NULL, size))
    {
        SpiCsHigh();// Deassert the CS to deselect the SD Card
        return 0;
    }
    ...
    }while((response & SDMMC_SPI_DR_MASK)!= SDMMC_SPI_DR_ACCEPTED);  
                                     ...
                                     uint32_t startBlock, uint32_t blockCount)
{
    ...
    if (blockCount == 1)
    {
        ...
        if (SDSPI_DRV_Write(buffer, card->blockSize, SDMMC_SPI_DT_START_SINGLE_BLK) != card->blockSize)
        {
            return kStatus_SDSPI_Failed;
        }
    }
    else
    {
        ...
        for (i = 0; i < blockCount; i++)
        {
            if (SDSPI_DRV_Write(buffer, card->blockSize, SDMMC_SPI_DT_START_MULTI_BLK) != card->blockSize)
            {
                return kStatus_SDSPI_Failed;
            }
            buffer += card->blockSize;
